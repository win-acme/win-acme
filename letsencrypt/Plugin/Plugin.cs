    ...
    public abstract class Plugin
    {
        ...
        protected virtual AuthorizationState Authorize(Target target, Options options)
        {
            ...
            foreach (var dnsIdentifier in dnsIdentifiers)
            {
                ...
                BeforeAuthorize(target, answerPath, httpChallenge.Token);
                ...
                finally
                {
                    ...
                    {
                        DeleteAuthorization(options, answerPath, httpChallenge.Token, webRootPath, filePath);
                    }
                }
            }
        ...
        internal virtual string GetCertificate(Target binding, AcmeClient client, Options options)
        {
            ...
            try
            {
                if (options.RSAKeyBits >= 1024)
                {
                    rsaPkp.NumBits = options.RSAKeyBits;
                }
                else
                {
            ...
            if (certRequest.StatusCode == HttpStatusCode.Created)
            {
                ...
                var csrPemFile = Path.Combine(options.CertOutPath, $"{filePrefix}-csr.pem");
                var crtDerFile = Path.Combine(options.CertOutPath, $"{filePrefix}-crt.der");
                var crtPemFile = Path.Combine(options.CertOutPath, $"{filePrefix}-crt.pem");
                var chainPemFile = Path.Combine(options.CertOutPath, $"{filePrefix}-chain.pem");
                string crtPfxFile = null;
                if (!options.CentralSsl)
                {
                    crtPfxFile = Path.Combine(options.CertOutPath, $"{filePrefix}-all.pfx");
                }
                else
                {
                    crtPfxFile = Path.Combine(options.CentralSslStore, $"{filePrefix}.pfx");
                }
        ...
        public virtual void RunScript(Target target, string pfxFilename, X509Store store, X509Certificate2 certificate, Options options)
        {
            if (!string.IsNullOrWhiteSpace(options.Script))
            {
                ...
                {
                    var parameters = string.Format(options.ScriptParameters, target.Host, options.PFXPassword,
