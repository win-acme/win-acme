...
namespace PKISharp.WACS.Clients.Acme
{
    ...
    /// </summary>
    internal class AcmeClientAuthorized 
    {
        ...
        internal async Task<AcmeChallenge> AnswerChallenge(AcmeChallenge challenge)
        {
            ...
            var tries = 1;
            while (
                challenge.Status == AcmeClient.AuthorizationPending ||
                challenge.Status == AcmeClient.AuthorizationProcessing)
            {
                if (challenge.Url == null)
                {
                    throw new NotSupportedException("Missing challenge url");
        ...
        internal async Task<AcmeOrderDetails> SubmitCsr(AcmeOrderDetails details, byte[] csr)
        {
            ...
            await WaitForOrderStatus(details, AcmeClient.OrderReady);
            if (details.Payload.Status == AcmeClient.OrderReady)
            {
                ...
                await WaitForOrderStatus(details, AcmeClient.OrderProcessing, true);
            }
            return details;
        }
